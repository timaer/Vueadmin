...
<script>
  export default {
    ...
    created(){
          this.initShopInfo(this.$store.state.user.shopid)
    },
    methods: {
      onSave() {
          if(this.processing){
          ...
          this.form.id=this.$store.state.user.shopid
          this.form.logo=this.form.logo[0]
          this.form.headimg=this.form.headimg[0]
          this.form.hotbg=this.form.hotbg[0]
          this.processing=true
          this.$post("/shop/saveShop",{shopinfo:this.form,goodsclass:this.dynamicTags}).then((data)=>{
               if(data.result.success){
                   this.$message({message:data.result.info,type:'success'})
                   setTimeout(()=>{this.initShopInfo(this.$store.state.user.shopid)},500)
               }else{
      ...
      initShopInfo(shopid){
          this.$post("/shop/getShopInfo",{shopid:shopid}).then((data)=>{
             if(data.result.success){
                ...
                this.dynamicTags=data.result.info.goodsclass
